...
create_certificate_per_user() {
    ...
    keytool -keystore kafka.$user.keystore.jks -alias $user -certreq -file $user.csr -storepass conduktor -keypass conduktor -ext "$san_for_csr" >> $log_file 2>&1
...
EOF
    ...
    keytool -noprompt -keystore kafka.$user.keystore.jks -alias snakeoil-caroot -import -file ${CA_PATH}/snakeoil-ca-1.crt -storepass conduktor -keypass conduktor >> $log_file 2>&1
    #keytool -list -v -keystore kafka.$user.keystore.jks -storepass conduktor

    # Sign and import the host certificate into the keystore
    keytool -noprompt -keystore kafka.$user.keystore.jks -alias $user -import -file $user-ca1-signed.crt -storepass conduktor -keypass conduktor -ext "SAN=dns:$user,dns:localhost" >> $log_file 2>&1
    #keytool -list -v -keystore kafka.$user.keystore.jks -storepass conduktor

    # Create truststore and import the CA cert
    # TODO: remove this after we migrated stacks to use 'global' truststore
    keytool -noprompt -keystore kafka.$user.truststore.jks -alias snakeoil-caroot -import -file ${CA_PATH}/snakeoil-ca-1.crt -storepass conduktor -keypass conduktor >> $log_file 2>&1
